...
def _v_breakeven(v: float) -> tuple[str, str, str]:
    ...
    if v < 2.0:
        return "BUY",  "#22cc66", "Inflation expectations anchored below 2% — no forced Fed tightening"
    if v < 2.5:
    ...
    return   "AVOID", "#ff2222", "Inflation de-anchored — historically bearish for bonds and growth stocks"
...
def _v_curve(v: float, label: str) -> tuple[str, str, str]:
    ...
    return   "AVOID", "#ff2222", f"{label} deeply inverted — strong recession signal (12–18 month lead)"
...
def _m2_analysis(obs: list[dict]) -> dict:
    ...
    base = {"label": "M2 Money Supply (YoY)", "series_id": "M2SL", "unit": "% YoY",
            "change_7d": None, "date": obs[0]["date"] if obs else None}
    if len(obs) < 13:
        return {**base, "current": None, "verdict": "HOLD", "color": "#ffaa00",
    ...
    try:
        ...
        if yoy > 5:
            v, c, s = "BUY",  "#22cc66", f"M2 expanding +{yoy:.1f}% YoY — liquidity tailwind for equities (6–12 month lead)"
        elif yoy > 0:
            v, c, s = "HOLD", "#ffaa00", f"M2 growing +{yoy:.1f}% YoY — mild liquidity expansion"
        elif yoy > -5:
            v, c, s = "SELL", "#ff6644", f"M2 contracting {yoy:.1f}% YoY — liquidity headwind for risk assets"
        else:
            v, c, s = "AVOID", "#ff2222", f"M2 sharply contracting {yoy:.1f}% YoY — historically precedes equity stress"
...
def _cross_asset() -> dict:
    ...
    try:
        ...
        if not cu_hist.empty and not au_hist.empty:
            ...
            if chg > 0:
                v, c, s = "BUY",  "#22cc66", "Copper/Gold rising — risk-on. Industrial demand outpacing safe-haven demand."
            else:
                v, c, s = "SELL", "#ff6644", "Copper/Gold falling — risk-off. Investors rotating into gold."
            result["copper_gold_ratio"] = {
                "label": "Copper/Gold Ratio", "current": ratio,
                "change_7d": chg, "unit": "×1000", "verdict": v, "color": c, "signal": s,
            }
    except Exception:
        pass
    ...
    try:
        ...
        if not dxy_hist.empty:
            ...
            if chg < -0.5:
                v, c, s = "BUY",  "#22cc66", "Dollar weakening — tailwind for multinationals, commodities, and EM assets"
            elif chg > 0.5:
                v, c, s = "SELL", "#ff6644", "Dollar strengthening — headwind for multinationals, commodities, and emerging markets"
            else:
                v, c, s = "HOLD", "#ffaa00", "Dollar range-bound — neutral cross-asset impact"
            result["dxy"] = {
                "label": "US Dollar Index (DXY)", "current": cur,
                "change_7d": chg, "unit": "index", "verdict": v, "color": c, "signal": s,
            }
    except Exception:
        pass
...
def get_fred_macro() -> dict:
    ...
    def _ind(series_id: str, label: str, unit: str, verdict_fn, days_back: int = 7) -> dict:
        ...
        if cur is None:
            return {"error": "unavailable", "label": label, "series_id": series_id,
                    "current": None, "change_7d": None, "unit": unit,
                    "verdict": "HOLD", "color": "#666", "signal": "Data unavailable",
                    "date": None}
        v, c, s = verdict_fn(cur)
        return {
            "label": label, "series_id": series_id,
            "current": round(cur, 4), "change_7d": chg, "unit": unit,
            "verdict": v, "color": c, "signal": s,
    ...
    return {
        ...
        "rates":           {"real_yield_10y": ry, "breakeven_10y": be,
                            "yield_curve_2s10s": c210, "yield_curve_3m10y": c310, "sofr": sofr},
        "liquidity":       {"m2": m2},
        "cross_asset":     cross,
