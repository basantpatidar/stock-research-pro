...
**Doc version:** 1.7 · **Last updated:** 2026-05-19
...
| Date | Change |
|---|---|
| 2026-05-19 | Dip scanner persist + notify fix (`fix/dip-scanner-persist-and-notify`): the every-5-min `_run_dip_scan` background job in `backend/app/services/scheduler.py` was firing WebSocket alerts only — no `scanner_alerts` DB row, no Telegram push, so today's live fires "vanished" until a `/dip-scanner/backfill` (re-seed-history) replayed them as `source="backtest"` rows. `_run_dip_scan` now mirrors the MCF flow: after the WS `broadcast(...)` for `best`, it calls `_save_alert(db, opp)` (the existing 15-min dedup gate from `backend/app/api/dip_scanner.py`) and then `notifier.send_scanner_alert(saved)`. `_save_alert` signature changed from `-> None` to `-> ScannerAlert | None` (returns the persisted row, or None when deduped) so the notifier gets the ORM object. Existing call site in `/dip-scanner/scan` ignores the return — backward compatible. |
| 2026-05-17 | Telegram bot Sprint 2 (`feat/telegram-commands`): multi-user + inbound commands. New `TelegramUser` DB model + migration `c3d4e5f6a7b8`. Registration: `/start <invite_code>` adds chat_id to DB; owner (`TELEGRAM_CHAT_ID`) auto-admin. New `telegram_handler.py` — long-poll loop (asyncio task), command router. User commands: `/help /status /mode [saver\|normal\|deep] /scan [loose] /alerts [N] /usage /watchlist /add /remove_ticker /pause [duration] /resume`. Admin commands: `/users /remove /promote /demote`. `notifier.py` now broadcasts to all active `telegram_users` rows (falls back to config `TELEGRAM_CHAT_ID` if table empty). New env: `TELEGRAM_INVITE_CODE` in `.env`. Polling started/stopped in FastAPI lifespan (`main.py`). |
